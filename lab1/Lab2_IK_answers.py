...
def CCD(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    while (np.linalg.norm(joint_positions[path[end_index]] - target_pose) >= 1e-2 and cnt <= 10):
        for i in range(end_index):
            current_index = end_index - i - 1   # 在每次 CCD 循环中，从末端执行器开始，依次向根节点方向遍历机器人的路径上的每个关节。
...
def part1_inverse_kinematics(meta_data, joint_positions, joint_orientations, target_pose):
    ...
    for i in range(len(meta_data.joint_parent)):
        ...
        if meta_data.joint_name[i] not in path_name:
            joint_positions[i] = joint_positions[meta_data.joint_parent[i]] + R.from_quat(joint_orientations[meta_data.joint_parent[i]]).apply(
                                     meta_data.joint_initial_position[i] - meta_data.joint_initial_position[meta_data.joint_parent[i]])
